...
func List(_ context.Context, deps Deps, in ListInput) (response.ListResult, error) {
	...
	if in.Cursor != "" {
		...
		matchesView := bytesEqual(cursorView, view)
	...
	progress.Start(context.Background(), "list", "Listing library contents", len(out))
	progress.Success(context.Background(), "list", fmt.Sprintf("Listed library contents (%d series)", len(out)), len(out))
...
func indexOfAnchor(rows []indexfile.Row, anchor []byte) int {
	for i, row := range rows {
		if bytesEqual(anchorHash(row.Series.String()), anchor) {
			return i
		}
	}
	return -1
