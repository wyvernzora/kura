...
func TestWriteReadPlanRoundTrip(t *testing.T) {
	...
	in := planfile.PlanRecord{
		...
		FileTitle:   textnorm.NFC("Bookworm"),
		Snapshot:    "abc123",
		Changes: []planfile.Change{
			{
				Kind:       "add",
				Episode:    episode,
	...
	if out.Token != in.Token || out.Series != in.Series || out.MetadataRef != in.MetadataRef {
		t.Fatalf("round-trip mismatch: %#v", out)
	}
	if len(out.Changes) != 1 || out.Changes[0].To != in.Changes[0].To {
		t.Fatalf("changes mismatch: %#v", out.Changes)
	}
	if len(out.Changes[0].Companions) != 1 {
		t.Fatalf("companion count = %d", len(out.Changes[0].Companions))
	}
}
...
func TestAppendLogMarksApplied(t *testing.T) {
	...
	if err := log.AppendMove(now, "after", 1, 1, planfile.FileMove{From: "a", To: "b"}, nil); err != nil {
		t.Fatal(err)
	}
