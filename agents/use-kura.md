...
- has a stable **metadata ref** like `tvdb:370070` (the primary handle
  for every tool except `resolve_series`),
...
`get_series` reads persisted metadata only — it does not probe the
filesystem. If a file disappeared after the last scan, the slot still
reads `present`. Run `scan_series` to reconcile disk state back to
metadata.

**Series-level rollups (`list_series`):**
...
|---|---|
| `resolve_series` | None. Looks up metadata refs from titles. |
| `list_series` | None. Returns one page of series with summary state. Paginated — default 100 rows/page, max 1000. See §12. |
| `get_series` | None. Returns full state of one series. |
| `list_inbox` | None. Lists files under the configured inbox root so you can discover what's available to stage. Structured output, mtime-desc sorted. See §4.d + §13. |
| `add_series` | Creates a new tracked series. Empty directory. |
| `import_series` | Marks an existing untracked directory as tracked. |
| `stage_series_media` | Queues a batch of staging changes against one series: episode stages, trash items, extras placements. Files are **not** moved until reconcile. **Async.** See §4.d. |
| `reset_series_staging` | Cancels queued stages by episode marker, trash ULID, extras ULID, or `--all`. No file change. |
| `plan_series_reconcile` | None visible. Computes what `reconcile_apply` would do, including stagedTrash + stagedExtras. |
| `apply_series_reconcile` | Moves staged files into place: episodes to canonical slots, stagedTrash to `.kura/trash/`, stagedExtras to `Season N/Extra/[prefix]/`. Displaces old files to trash. **Async.** |
| `scan_series` | Re-reads the series's directory and updates which file is recorded for each slot. **Async.** |
| `get_job` | None. Polls an async job. |

**Async tools** return a `jobId` immediately. Poll `get_job`
...
4. **Always get a metadata ref from `resolve_series` first** unless you
   already have one from `list_series` / `get_series`. Never invent or
   guess refs.
5. **Tools that take `ref` accept exactly one ref**, not a title.
6. **`resolve_series` cardinality drives behavior:** 0 → no match,
   surface to user. 1 → safe to act. 2+ → disambiguate before acting
   (see §5).
7. **After `scan_series` or `apply_series_reconcile`**, poll the job
...
10. **Never string-match library rows against external release
    titles.** This includes inbox filenames (`list_inbox`),
    raw release titles from RSS feeds, web-search results,
    tracker listings, IRC announce, manual user-pasted strings —
    anything that isn't already a MetadataRef. `list_series` returns
    ...
    `resolve_series` to obtain a MetadataRef, then `get_series` to
...
11. **User references to inbox locations are `inbox:` selectors.**
    ...
    the relative path to `list_inbox(path=<rel>)` to scope a
...
```
list_series                                 # page 1 (default 100 rows)
list_series(cursor=<nextCursor>)            # repeat until nextCursor is absent
```
...
`get_series(ref)` over re-listing for a single known series.

Group rows by `status`. Recommended attention order:

1. `error` — fix or surface to user.
2. `incomplete` — investigate per series with `get_series`.
...
```
resolve_series(terms=[<directory or title>])
# pick the right candidate — see §5
import_series(ref=<chosen>, directory=<existing directory>)
scan_series(ref=<chosen>) → jobId
get_job(jobId)
```
...
```
resolve_series(terms=[<title>])
add_series(ref=<chosen>)         # creates the directory
...
`stage_series_media` queues a batch of changes against one series in a single
...
byte-for-byte from `list_inbox` output. Never paraphrase,
...
**Source when files lack tags.** `stage_series_media` runs mediainfo but
...
list_inbox(path="[BDrip] Show Title")
# → copy the inbox:<rel> selector you want to stage.

jobId = stage_series_media(
  ref=<series>,
...
get_job(jobId)                          # poll until terminal

plan_series_reconcile(ref=<series>) → {token, plan}
apply_series_reconcile(ref=<series>, token=<token>) → jobId
get_job(jobId)
get_series(ref=<series>)                         # verify
```
...
`reset_series_staging(ref, episode=<marker> | trash=[<ulid>...] | extras=[<ulid>...] | all=true)`.
...
resolve_series(terms=[<title A>])    resolve_series(terms=[<title B>])   ...

# For each series, decide add vs already-tracked. Run in parallel.
# Prefer get_series first when uncertain — add_series errors if the
# series exists, and a not_found from get_series is the cheapest
# discriminator.
get_series(ref=<A>) || add_series(ref=<A>)    get_series(ref=<B>) || add_series(ref=<B>)   ...

# Stage every series in parallel; each is its own batch + its own
# job. Poll all jobIds until terminal before continuing.
stage_series_media(ref=<A>, episodes=[...])    stage_series_media(ref=<B>, episodes=[...])   ...

# Plan every series in parallel. Tokens are series-scoped.
plan_series_reconcile(ref=<A>)    plan_series_reconcile(ref=<B>)   ...
...
# the others.
apply_series_reconcile(ref=<A>, token=<tokenA>)    apply_series_reconcile(ref=<B>, token=<tokenB>)   ...
```
...
series, prefer `stage_series_media` + `plan_series_reconcile` /
`apply_series_reconcile` over relying on a re-scan to adopt them. Use
...
- **Inbox items** — `list_inbox` filenames the user has
  already downloaded. Scope the listing to a subdir when the user
  narrows the request (e.g. "inbox/pending" → `list_inbox(path="pending")`,
  per rule §3.11).
...
resolve_series(terms=[<release title or inbox name or fragment>])
...
get_series(ref=<chosen metadata ref>)
...
# - Untracked or no overlap → candidate to adopt (add_series or import_series).
...
contents into the right `stage_series_media` arrays:
...
`list_series` output and any source of raw release titles
(`list_inbox`, RSS, web-search, tracker JSON, etc.) by
...
## 5. Disambiguating `resolve_series`

See the `resolve_series` tool description for candidate field meanings
...
See the `scan_series` tool description for `synced[]` / `skipped[]` /
...
the loser trashes into a single `stage_series_media` call. The winner comes
...
- **Stage + reconcile** is for any case scan can't (or shouldn't)
  ...
  - File lives in the configured inbox and you want to
    install it. Discover via `list_inbox`, stage with `inbox:`
    selector.
...
`plan_series_reconcile` computes what `reconcile_apply` would do and
returns a `token`. Nothing changes on disk. `apply_series_reconcile`
...
**Apply re-validates.** `apply_series_reconcile` re-reads
...
- Plan and apply in the same flight is still the cheap default —
  ...
  in between.
- If apply returns `stale_snapshot`, call `plan_series_reconcile`
...
|---|---|---|
| `resolve_series` returns 0 | Title romanization mismatch or provider gap. | Try alternate titles; ask the user for an explicit ref if they have one. |
| `scan_series` skips good-looking files with `metadata_slot_missing` | Provider numbers seasons differently than the user's filenames. | Inspect with `get_series`; user may need to renumber files or stage explicitly. |
| Source column shows resolution string (e.g. `1920x1080`) | Filename suffix has only resolution, no source token. Records show `Unknown` correctly on fresh scans of recent binaries. | Re-scan; if still wrong, override via `stage_series_media(source=...)`. |
| `plan_series_reconcile` shows the active or staged file as `Unknown` source | Kura's filename parser missed the source token on the originally-staged file (uncommon naming, parser gap). | Check the **original** filename (before kura's rename) for a source token (`BluRay`, `WebRip`, `WEBDL`, `HDTV`, etc.). If present, re-stage the affected file with an explicit source override using a `series:` media selector (in-place override): `stage_series_media` episode item with `media: "series:<active-path>"`, `replace: true`, `source: "<token>"`, no companions. Reconcile_apply rewrites the persisted source and renames the file to its corrected canonical filename without moving it through the inbox. |
| `stage_series_media` errors with episode-already-exists | Slot already has a recorded file at a different path. | Confirm with user, then re-call with `replace: true` on the episode item. |
| `stage_series_media` rejects with "expected inbox: scheme" / "expected series: scheme" | Selector type wrong for the field. Episode `media` accepts `inbox:` normally and `series:` only for in-place metadata override; episode companions and extras `source` need `inbox:` selectors; trash `path` + companions need `series:` selectors. | Re-build the selector with the right scheme. |
| `stage_series_media` rejects with "missing scheme" / "selector escapes root" / "leading slash not allowed" | Bare path or absolute path passed where a selector was expected. Selectors look like `inbox:<rel>` or `series:<rel>`, never `/abs/path`. | Discover the right selector via `list_inbox`; rebuild and retry. |
| `list_inbox` returns `path does not exist` | Subpath doesn't exist under the configured inbox root. | List the parent dir; correct the path. |
| `stage_series_media` rejects whole batch with `invalid_params` (Phase 1) | Trash invariant violated (path is active record / companion / inside a staged record), duplicate episode in batch, duplicate path across batch, or extras destination collision. Error message names the offender. | Fix the offending item and re-submit the whole batch. |
| `stage_series_media` returns success but with `skipped[]` entries (Phase 2) | Per-item probe failed (mediainfo, file vanished mid-flight). The rest of the batch was applied. | Inspect the `code` per skipped row; re-stage the failed items individually. |
| `apply_series_reconcile` fails as busy | Prior reconcile crashed and left a claim. | Route user to `kura reconcile recover <ref>` (CLI). |
| Series shows `error` in `list_series` | Library couldn't read the series. | `get_series(ref)` surfaces the reason; usually requires user intervention. |
| `list_series` returns `server_not_ready` | Index is rebuilding (cold start, schema mismatch, or corruption recovery). | Wait a few seconds and retry. Don't tight-loop. |
| `list_series` returns `invalid_cursor` | Cursor corrupt, or anchor row removed in a way that can't resume. | Restart from page 1 (no cursor). |
...
- Don't paste raw `get_series` JSON. Project to the fields the user
  asked about.
...
## 12. Paginating `list_series`

See the `list_series` tool description for full pagination rules. Key
...
See the `stage_series_media` tool description for the full selector table
and scheme rules.
