...
import (
	clipkg "github.com/wyvernzora/kura/cli/internal/cli"
...
func walkListPages(rt *runContext, c *client.Client, statuses []string, airing *bool, tags []string) (api.ListResult, bool, error) {
	var all api.ListResult
	cursor := ""
	drifted := false
	for {
		page, err := c.ListSeries(rt.Context, statuses, airing, tags, listPageSize, cursor)
		if err != nil {
			return api.ListResult{}, false, err
		}
		if page.DataChanged {
			drifted = true
		}
