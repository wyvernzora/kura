...
func (cmd *reconcileApplyCmd) Run(rt *runContext) error {
	...
	if err != nil {
		var applyResult api.ReconcileApplyJobResult
		if raw != nil {
...
// registry stores the raw api.ReconcileApplyJobResult JSON; CLI converts to
// api.ReconcileApply for render.
func applyResultToResponse(result api.ReconcileApplyJobResult) api.ReconcileApply {
	...
	if result.FailedStep != nil {
		...
		out.FailedStep = &api.FailedReconcileStep{
			ID:         fs.ID,
			Kind:       string(fs.Kind),
