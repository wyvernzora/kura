...
func TestImportCmdResolveTerms(t *testing.T) {
	tests := []struct {
		...
		{
			name: "directory only",
			cmd:  importCmd{Directory: "whatever"},
			want: []string{"whatever"},
		},
		{
			name: "directory plus text terms",
			cmd:  importCmd{Directory: "whatever", Terms: []string{"foo", "bar"}},
			want: []string{"whatever", "foo", "bar"},
		},
		{
			name: "empty extra terms",
			cmd:  importCmd{Directory: "whatever", Terms: []string{"", "  "}},
			want: []string{"whatever", "", "  "},
		},
		{
			name: "metadata ref term",
			cmd:  importCmd{Directory: "whatever", Terms: []string{"tvdb:370070"}},
		...
		},
		{
			...
			cmd:  importCmd{Directory: "whatever", Terms: []string{"foo", "tvdb:370070"}},
			want: []string{"foo", "tvdb:370070"},
		},
		{
			name: "multiple metadata ref terms",
			cmd:  importCmd{Directory: "whatever", Terms: []string{"tvdb:370070", "tvdb:999999"}},
			want: []string{"tvdb:370070", "tvdb:999999"},
		},
		{
			name: "dir term treated as text",
			cmd:  importCmd{Directory: "whatever", Terms: []string{"dir:tracked"}},
