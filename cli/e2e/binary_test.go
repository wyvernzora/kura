...
// The server and CLI binaries are built once for the whole test process.
// Cached via buildOnce.
var (
	e2eServerPath string
	e2eCLIPath    string
	buildOnce     sync.Once
	buildErr      error
)
...
// Subsequent calls return the cached paths. On compile failure, every
// caller surfaces the same error.
func buildKuraE2E(t *testing.T) (serverPath, cliPath string) {
	t.Helper()
	buildOnce.Do(func() {
		...
		root := repoRoot()
		serverBin := filepath.Join(dir, "kura-e2e-server")
		cmd := exec.Command("go", "build", "-tags=e2e_stub", "-o", serverBin, "./cmd/kura-library-manager")
		cmd.Dir = filepath.Join(root, "services", "library-manager")
		var stderr bytes.Buffer
		cmd.Stderr = &stderr
		if err := cmd.Run(); err != nil {
			buildErr = fmt.Errorf("go build kura-e2e-server: %w\nstderr: %s", err, stderr.String())
			return
		}
		cliBin := filepath.Join(dir, "kura")
		cmd = exec.Command("go", "build", "-o", cliBin, "./cmd/kura")
		cmd.Dir = filepath.Join(root, "cli")
		stderr.Reset()
		cmd.Stderr = &stderr
		if err := cmd.Run(); err != nil {
			buildErr = fmt.Errorf("go build kura: %w\nstderr: %s", err, stderr.String())
			return
		}
		e2eServerPath = serverBin
		e2eCLIPath = cliBin
	})
	if buildErr != nil {
...
// startDaemon builds both binaries (if needed) and boots
// `kura-e2e-server --config=... --use-test-stubs` against libRoot.
...
func startDaemon(t *testing.T, libRoot, inboxRoot, umask string) *e2eBinary {
	t.Helper()
	serverBin, cliBin := buildKuraE2E(t)
...
// run invokes the kura CLI binary as a subprocess with
