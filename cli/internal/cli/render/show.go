...
func Show(w io.Writer, result api.Show, asJSON bool) error {
	if asJSON {
...
func writeShowHeader(w io.Writer, result api.Show, styled bool) error {
...
func writeShowSeasons(w io.Writer, result api.Show) error {
...
// skipped silently.
func writeShowStaged(w io.Writer, result api.Show) error {
...
func writeStagedTrashTable(w io.Writer, items []api.TrashItemShow) error {
	tw := table.NewWriter()
...
func writeStagedExtrasTable(w io.Writer, items []api.ExtraItemShow) error {
	tw := table.NewWriter()
...
func writeShowSeasonTable(w io.Writer, seriesRoot string, episodes []api.EpisodeShow) error {
	styled := style.ShouldStyle(w)
	tw := table.NewWriter()
	...
	for _, episode := range episodes {
		if episode.Active != nil && episode.Staged != nil {
			appendShowRows(tw, seriesRoot, episode.Episode, episode.Aired, episode.PreferredTitle, api.StatusPresent, episode.Active, styled, true)
			appendShowRows(tw, seriesRoot, episode.Episode, episode.Aired, episode.PreferredTitle, api.StatusStaged, episode.Staged, styled, false)
			continue
		}
...
func appendShowRows(tw table.Writer, seriesRoot string, episode refs.Episode, aired, title string, status api.Status, media *api.MediaShow, styled bool, retired bool) {
...
func showEpisodeRow(seriesRoot string, episode refs.Episode, aired, title string, status api.Status, media *api.MediaShow, styled bool, retired bool) table.Row {
	source := ""
	resolution := ""
	file := ""
	if media != nil {
...
func displaySource(source string, status api.Status) string {
	if source == "Unknown" && (status == api.StatusMissing || status == api.StatusPending) {
		return ""
	}
	return source
}

func firstShowMedia(episode api.EpisodeShow) *api.MediaShow {
