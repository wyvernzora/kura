...
// and expiry summary.
func PlanReconcile(w io.Writer, result api.ReconcilePlan, asJSON bool) error {
	if asJSON {
...
func ApplyReconcile(w io.Writer, result api.ReconcileApply, asJSON bool) error {
	if asJSON {
		encoder := json.NewEncoder(w)
		encoder.SetIndent("", "  ")
		return encoder.Encode(result)
...
func RecoverReconcile(w io.Writer, result api.RecoverReconcile, asJSON bool) error {
	if asJSON {
		encoder := json.NewEncoder(w)
		encoder.SetIndent("", "  ")
		return encoder.Encode(result)
...
func flattenPlanMoves(changes []api.ReconcileChange) []api.ReconcileMove {
	var moves []api.ReconcileMove
	for _, change := range changes {
		if change.Replaced != nil {
			moves = append(moves, api.ReconcileMove{From: change.Replaced.From, To: change.Replaced.To})
			moves = append(moves, change.Replaced.Companions...)
		}
		moves = append(moves, api.ReconcileMove{From: change.From, To: change.To})
