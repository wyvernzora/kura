...
import (
	...
	"github.com/wyvernzora/kura/services/library-manager/pkg/api/media"
)

// EpisodeStatus colors a api.Status string. Used by show, scan, and
// reconcile renders.
func EpisodeStatus(status string, styled bool) string {
	...
	switch value {
	case string(api.StatusMissing):
		return Orange(value)
	case string(api.StatusPresent):
		return Green(value)
