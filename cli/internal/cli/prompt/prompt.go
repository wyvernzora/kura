...
func SelectCandidate(io stdio.Stdio, terms []string, candidates []api.Candidate) (api.Candidate, error) {
	if len(candidates) == 0 {
		return api.Candidate{}, errors.New("prompt: no candidates to select from")
	}
	if !io.IsInteractive() {
		WriteHint(io.Err, candidates)
		return api.Candidate{}, ErrNonInteractive
	}
	...
	if selected == noneOption {
		return api.Candidate{}, ErrCancelled
	}
...
func WriteHint(w io.Writer, candidates []api.Candidate) {
	if w == nil {
		return
	}
	if len(candidates) > 5 {
...
// hint.
func FormatCandidate(c api.Candidate) string {
