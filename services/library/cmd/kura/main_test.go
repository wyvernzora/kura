//go:build legacy_cli_tests

// Legacy CLI tests exercised the old direct-disk verb architecture.
// After the REST migration (see scratch/design/rest.md), CLI verbs go
// through internal/cli/client to a running kura serve --rest. End-to-
// end coverage now lives in e2e/ (subprocess harness against a real
// daemon). Kept under the legacy_cli_tests build tag for reference
// until rewritten or deleted.
...
func TestPathCommandPrintsSeriesRoot(t *testing.T) {
	server := newCLITestServer(t)
	defer server.Close()
...
func TestPathCommandSeriesFileFlag(t *testing.T) {
	server := newCLITestServer(t)
	defer server.Close()
	...
	want := filepath.Join(seriesDir, ".kura", "series.json") + "\n"
	if stdout.String() != want {
		t.Fatalf("stdout = %q, want %q", stdout.String(), want)
	}
}
...
func TestReconcileApplyCommandAppliesPlanToken(t *testing.T) {
	...
	if got := int(result["appliedSteps"].(float64)); got != 1 {
		t.Fatalf("appliedSteps = %d, want 1", got)
	}
	if got := int(result["totalSteps"].(float64)); got != 1 {
		t.Fatalf("totalSteps = %d, want 1", got)
	}
	if _, ok := result["failedStep"]; ok {
		t.Fatalf("failedStep present on success: %v", result["failedStep"])
	}
	...
	// Plan v2: header + N step lines + N event lines + 1 result line.
	// Single-episode self-refresh case: 1 step → 4 total.
	lines := strings.Split(strings.TrimSpace(string(data)), "\n")
	if len(lines) < 4 {
		t.Fatalf("plan log lines = %d, want >= 4\n%s", len(lines), string(data))
	}
	if !strings.Contains(string(data), `"type":"header"`) ||
		!strings.Contains(string(data), `"type":"step"`) ||
		!strings.Contains(string(data), `"type":"event"`) ||
		!strings.Contains(string(data), `"status":"success"`) {
		t.Fatalf("plan log missing expected records:\n%s", string(data))
	}
}
...
func TestParseStageEpisodeAcceptsMarkerAndStorageRef(t *testing.T) {
	...
	for input, want := range cases {
		got, err := refs.ParseEpisodeMarker(input)
		if err != nil {
			t.Fatalf("refs.ParseEpisodeMarker(%q): %v", input, err)
		}
		if got.String() != want {
			t.Fatalf("refs.ParseEpisodeMarker(%q) = %s, want %s", input, got, want)
		}
	}
}
