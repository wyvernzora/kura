...
func TestSubmit_NewJobIsTrackedAndStartsRunning(t *testing.T) {
	...
	j := jobs.Submit(r, context.Background(), jobs.KindScan, mustSeries(t, "show-a"), func(ctx context.Context) (int, error) {
		close(started)
		<-finish
		return 42, nil
	})
...
func TestSubmit_FailedTerminalCarriesError(t *testing.T) {
	...
	j := jobs.Submit(r, context.Background(), jobs.KindScan, mustSeries(t, "show-b"), func(ctx context.Context) (int, error) {
		return 0, want
	})
...
func TestSubmit_SameKindDeDupesReturnsExistingJob(t *testing.T) {
	...
	finish := make(chan struct{})
	var spawned int32
	first := jobs.Submit(r, context.Background(), jobs.KindScan, mustSeries(t, "show-c"), func(ctx context.Context) (int, error) {
		atomic.AddInt32(&spawned, 1)
		<-finish
		return 7, nil
	})
	second := jobs.Submit(r, context.Background(), jobs.KindScan, mustSeries(t, "show-c"), func(ctx context.Context) (int, error) {
		atomic.AddInt32(&spawned, 1)
		return 99, nil
	})
	if first != second {
...
func TestSubmit_CrossKindReturnsBusyFailed(t *testing.T) {
	r := newTestRegistry(t, jobs.Config{})
	finish := make(chan struct{})
	first := jobs.Submit(r, context.Background(), jobs.KindScan, mustSeries(t, "show-d"), func(ctx context.Context) (int, error) {
		<-finish
		return 1, nil
	})
	second := jobs.Submit(r, context.Background(), jobs.KindReconcileApply, mustSeries(t, "show-d"), func(ctx context.Context) (int, error) {
...
func TestSubmit_AfterTerminalAllowsResubmission(t *testing.T) {
	r := newTestRegistry(t, jobs.Config{})
	first := jobs.Submit(r, context.Background(), jobs.KindScan, mustSeries(t, "show-e"), func(ctx context.Context) (int, error) {
		return 1, nil
	})
	first.Wait(context.Background())
	// First terminal; bySeries entry should be cleared. Second submission spawns fresh.
	second := jobs.Submit(r, context.Background(), jobs.KindScan, mustSeries(t, "show-e"), func(ctx context.Context) (int, error) {
		return 2, nil
	})
...
func TestSubmit_TimeoutTerminatesWithJobTimeoutError(t *testing.T) {
	...
	j := jobs.Submit(r, context.Background(), jobs.KindScan, mustSeries(t, "show-f"), func(ctx context.Context) (int, error) {
		<-ctx.Done()
		return 0, ctx.Err()
	})
	_, err := j.Wait(context.Background())
...
func TestSubmit_ProgressCapturedOnJob(t *testing.T) {
	r := newTestRegistry(t, jobs.Config{})
	j := jobs.Submit(r, context.Background(), jobs.KindScan, mustSeries(t, "show-g"), func(ctx context.Context) (int, error) {
...
func TestSubmit_ProgressForwardedToCallerReporter(t *testing.T) {
	// Capture + relay contract: a reporter installed on the *caller's*
	// ctx (the one passed to Submit) receives every job-goroutine
	// emission, alongside the per-job capture for polling. The
	// registry's parent ctx is independent — Shutdown lifecycle, not a
	// reporter source.
	r := jobs.NewRegistry(context.Background(), jobs.Config{}, nil)
	t.Cleanup(func() { r.Shutdown(2 * time.Second) })

	var seen int32
	callerCtx := progress.With(context.Background(), func(_ context.Context, _ progress.Event) {
		atomic.AddInt32(&seen, 1)
	})

	j := jobs.Submit(r, callerCtx, jobs.KindScan, mustSeries(t, "show-i"), func(ctx context.Context) (int, error) {
	...
	if got := atomic.LoadInt32(&seen); got == 0 {
		t.Fatal("caller reporter saw 0 events; relay should forward job-goroutine emissions")
	}
}

func TestSubmit_NoCallerReporter_NoForwarding(t *testing.T) {
	// Negative: a Submit with a bare context.Background() callerCtx
	// must not forward to any reporter. Registry parent reporter (if
	// any) is intentionally ignored after the per-call refactor.
	var registryReporterFired int32
	registryParent := progress.With(context.Background(), func(_ context.Context, _ progress.Event) {
		atomic.AddInt32(&registryReporterFired, 1)
	})
	r := jobs.NewRegistry(registryParent, jobs.Config{}, nil)
	t.Cleanup(func() { r.Shutdown(2 * time.Second) })

	j := jobs.Submit(r, context.Background(), jobs.KindScan, mustSeries(t, "show-i2"), func(ctx context.Context) (int, error) {
		progress.Update(ctx, "scan", "midway", 1, 1)
		return 0, nil
	})
	j.Wait(context.Background())

	if got := atomic.LoadInt32(&registryReporterFired); got != 0 {
		t.Fatalf("registry-parent reporter fired %d times; should be ignored under per-caller contract", got)
	}
}
...
func TestShutdown_CancelsInFlightJobs(t *testing.T) {
	...
	jobs.Submit(r, context.Background(), jobs.KindScan, mustSeries(t, "show-h"), func(ctx context.Context) (int, error) {
		close(started)
		<-ctx.Done()
		close(done)
		return 0, ctx.Err()
