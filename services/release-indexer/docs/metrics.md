...
| `takuhai_source_crawls_total` | counter | `source`, `result` | Scheduled crawls by source and `ok`, `crawl_error`, or `ingest_error`. |
| `takuhai_source_crawl_duration_seconds` | histogram | `source` | End-to-end scheduled crawl and ingest duration. |
| `takuhai_source_crawl_posts_total` | counter | `source` | Posts returned by scheduled crawls. |
