...
| `exhausted -> suppressed` applies with one audit row carrying the reason, preserves `attempt_count`, refuses a ref (`ErrRefForbidden`), is a no-op when repeated, and `unmatched -> suppressed` stays `ErrInvalidTransition` (submit-only) | `TestOperatorStatus_SuppressAnExhaustedRelease` |
