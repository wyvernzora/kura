...
// buildKuraE2E compiles cmd/kura-library-manager with -tags=e2e_stub once per process,
...
func buildKuraE2E(t *testing.T) string {
	...
	buildOnce.Do(func() {
		...
		cmd := exec.Command("go", "build", "-tags=e2e_stub", "-o", bin, "./cmd/kura-library-manager")
		cmd.Dir = repoRoot()
