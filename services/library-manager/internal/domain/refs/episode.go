...
func ParseEpisode(value string) (Episode, error) {
	...
	// Zero-value round-trip: Episode{} marshals as "S00E0000"; accept
	// that form back as the zero value so JSON encoders/decoders can
	// pass refs.Episode through wire formats without losing fidelity
	// when the field is unset (e.g. trash entries not tied to an
	// episode slot).
