...
Polling cadence is up to the caller. Terminal jobs stay in memory for `jobs.retention` (default `30m`) and may also be readable from persisted job logs until log retention removes them. A lookup returns `not_found` only when neither source has the job ID.
